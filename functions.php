...
/**
 * Generate the full robots.txt response.
 *
 * Replaces WordPress's default body (not just the sitemap line) so
 * the file is fully version-controlled from this theme. The static
 * /robots.txt on Plesk MUST be removed for this filter to take
 * effect — WP only serves the virtual file when no static file is
 * present at webroot.
 *
 * Two design decisions worth noting:
 *
 *   1. Per-bot Disallow blocks repeat themselves. robots.txt spec
 *      says: once a specific User-agent group matches, rules from
 *      `*` are IGNORED. So if we list `User-agent: GPTBot \n
 *      Allow: /` and nothing else, GPTBot will happily crawl
 *      /wp-admin/, search results, trackbacks, etc — wasting
 *      crawl budget on zero-value paths. The repetition is the
 *      cost of explicit per-bot control.
 *
 *   2. We do NOT block /wp-json/. Some plugins (Rank Math sitemaps,
 *      Contact Form 7, page builders) load REST resources during
 *      page render. Blocking the path would cause Googlebot to
 *      render an incomplete view of the site. Admin endpoints
 *      under /wp-json/ are protected at the WP capability layer,
 *      not by robots.txt.
 *
 *   3. We do NOT block /*?utm_. Canonical tags collapse UTM
 *      variants to the canonical URL; the disallow only hides
 *      campaign-sourced backlinks from Googlebot's reachability
 *      graph and gains nothing in return.
 */
function hashbox_robots_txt( $output, $public ) {
    ...
    $sitemap = hashbox_rank_math_is_active()
        ? home_url( '/sitemap_index.xml' )
        : home_url( '/wp-sitemap.xml' );

    // Shared block — applied identically to `*` and every AI bot.
    $disallow_block = implode( "\n", array(
        'Disallow: /wp-admin/',
    ...
    // AI / GEO crawlers we explicitly opt in. Listed individually
    // so the next person editing this file can see exactly which
    // surface the site appears on. OpenAI / Perplexity split their
    // training crawler from live-browsing and search-index agents
    // in 2024 — each one needs its own group.
    $ai_bots = array(
        'GPTBot',              // OpenAI training
        'ChatGPT-User',        // OpenAI live browsing during chat
        'OAI-SearchBot',       // ChatGPT Search index (Oct 2024)
        'ClaudeBot',           // Anthropic training
        'anthropic-ai',        // Anthropic alt UA
        'Claude-Web',          // Claude live browsing
        'Google-Extended',     // Gemini / Google AI Overviews opt-in
        'PerplexityBot',       // Perplexity index
        'Perplexity-User',     // Perplexity live answer fetch
    ...
    $output  = "# Hashbox Studio robots.txt — generated by theme filter\n";
    $output .= "# Source of truth: hashbox_robots_txt() in functions.php\n";
    $output .= "# If editing the static /robots.txt on Plesk has no effect,\n";
    $output .= "# that's because this filter overrides it (remove the static file).\n\n";

    $output .= "User-agent: *\n{$disallow_block}\n\n";

    $output .= "# ---------------------------------------------------------------\n";
    $output .= "# AI / GEO crawlers — explicitly opted in.\n";
    $output .= "# Disallow rules are re-stated per group because robots.txt spec\n";
    $output .= "# ignores * rules once a specific user-agent group matches.\n";
    $output .= "# ---------------------------------------------------------------\n\n";
