...
## 10. หน้า Landing แบบ Message-Matched (ดัน Quality Score = CPC ถูกลง)

`page-ads-website.php` สลับหัวข้อ/ข้อความให้ตรงกลุ่มอัตโนมัติ **ตาม slug ของหน้า**
ยิ่งหน้าตรงกับคีย์เวิร์ดที่ลูกค้าเสิร์ช Google ยิ่งให้ Quality Score สูง = **CPC ถูกลง
ได้คลิกมากขึ้นด้วยงบ 200 เท่าเดิม** และ conversion rate ดีกว่าหน้ารวม

วิธีใช้ — สร้างหน้าใน WordPress โดยตั้ง slug ให้มีคำเหล่านี้ แล้วเลือก Template เดียวกัน
("Ads Landing: รับทำเว็บไซต์"):

| Slug มีคำว่า | หน้าจะปรับเป็น | ใช้คู่กับ Ad Group |
|---|---|---|
| (อื่น ๆ ทั่วไป) | รับทำเว็บไซต์บริษัท ทั่วไป | Group 1 (Core) + 2 (SEO) |
| `corporate` หรือ `b2b` | รับทำเว็บไซต์องค์กร / B2B | Group 3 (Corporate) |
| `ecommerce` / `shop` / `store` | รับทำเว็บไซต์ขายของ / E-commerce | Group 4 (E-commerce) |

ตัวอย่าง: สร้างหน้า slug `รับทำเว็บไซต์-corporate` → ชี้ Final URL ของ Ad Group 3 มาที่หน้านี้
ส่วน source ของ lead จะถูกแท็กแยก (`google-ads-corporate`, `google-ads-ecommerce`)
ในอีเมลแจ้งเตือนและใน dataLayer ทำให้ดูได้ว่ากลุ่มไหนสร้าง lead

> เริ่มจากหน้าทั่วไปหน้าเดียวก่อนก็ได้ พอ Ad Group 3/4 เริ่มมีคลิก ค่อยแตกหน้าเพิ่ม

## 11. หลังส่งฟอร์ม = ดันเข้า LINE ทันที (เร่ง Speed-to-Lead)

เมื่อลูกค้าส่งฟอร์มสำเร็จ หน้าไม่ได้แค่ขึ้น "ขอบคุณ" แต่จะ **ดันปุ่มเพิ่มเพื่อน LINE OA
+ ปุ่มโทร** ให้ทักทันที เพราะ lead ที่คุยได้ใน 5 นาทีปิดง่ายกว่าหลายเท่า
แนะนำตั้ง **greeting message ใน LINE OA** ให้ตอบอัตโนมัติทันทีที่เพิ่มเพื่อน
(เช่น "ขอบคุณที่สนใจ ขอ URL เว็บปัจจุบันเพื่อเริ่ม Audit ฟรีได้เลยครับ")
