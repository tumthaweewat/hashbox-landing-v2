...
- ✅ **GBP เติมข้อมูลแล้ว (Claude ทำใน Search merchant panel, Tum อนุมัติ 2026-08-30):** description ใหม่ 469 ตัวอักษร (5 บริการ + ราคาปัจจุบัน แทนของเดิม "3 ด้านหลัก / เริ่ม 80,000") · หมวดเพิ่ม Marketing agency + Internet marketing service (รวม 5: Web Designer หลัก, Software company, Marketing consultant, +2) · Services 5 รายการชื่อตรงเว็บ + ราคา From + คำอธิบาย: รับทำเว็บไซต์ SEO-Ready ฿35,900 / รับทำ SEO ฿29,900 / รับทำ AI Search (GEO) quote / ที่ปรึกษา AI สำหรับธุรกิจ ฿60,000 / รับทำ Workflow Automation n8n quote · โพสต์ #1 การันตี "ไม่โต ไม่จ่าย" + ปุ่ม Learn more → /services/seo/#guarantee เผยแพร่แล้ว (ไม่มีรูป) · ทุก edit อยู่ใน pending review (≤1 วัน) · โพสต์ #2–#4 ตั้งเวลาแล้ว (Scheduled): #2 AI Mode 6 ก.ย. → /google-ai-mode-คืออะไร-2026/ · #3 10 บริษัท SEO 15 ก.ย. → /บริษัทรับทำ-seo-ไทย-2026/ · #4 GEO checker 22 ก.ย. 10:00 → /geo-checker/ (ทุกโพสต์ยังไม่มีรูป — 6 ไฟล์รูปส่งให้ Tum แล้ว: logo-720, cover-og, guarantee, geo-checker, psi-report, ai-mode-post) · **ยังไม่ได้ทำ (Tum):** อัปโหลดรูป 6 ไฟล์ + ใส่รูปในโพสต์ที่ตั้งเวลาไว้, รีวิว (ขอเอง) · Q&A ทำไม่ได้ (โปรไฟล์นี้ไม่มี section Q&A) · เทคนิค: category autocomplete ต้องพิมพ์ทีละตัวด้วย key action + UI ภาษาอังกฤษ (hl=en); dialog ไม่ scroll ด้วย wheel ใช้ Tab focus แทน; date/time/link field ต้องพิมพ์ด้วย keystroke จริง (JS value setter → aria-invalid) — วิธีที่เวิร์ก: JS clear+focus แล้ว type; ปุ่ม Learn more เลือกด้วย JS .click() บน role=option ได้
...
- ✅ **C6/T3 (2026-08-29):** Tum ยืนยันใช้ชื่อเคสบนเว็บ (ไม่เปิดชื่อจริง) + ตัวเลขที่เปิดได้ = **Lighthouse 90+ / 20+ keyword หน้าแรก / 90 วัน** → เพิ่ม block `proof` ใน `hashbox_render_case_study()` ("ตัวเลขที่ยืนยันได้ — เกณฑ์เดียวกับการันตี") บน /work/nexus-corp/ + /work/rank-project/ · /services/seo/ + /en/seo/ guarantee section ลิงก์ไปเคส · AutoBot LINE ไม่ใส่ (ตัวเลขชุดนี้ไม่ตรงกับงาน bot) · Tum ตัดสินใจ 2026-08-29: **คงตัวเลขเดิมทั้งหมด** ('ปรับให้ตรงกับตัวเดิม') → proof ของ Nexus ใช้ Lighthouse 100 ตามหน้าเดิม (เกณฑ์การันตี 90+ อยู่ใน note); results ชุด +540/+700/+2,200 บนสองเคสคงไว้ตามเดิม
- ✅ **"ส่วนที่เหลือ" (Tum 2026-08-29 ค่ำ):** (1) วิเคราะห์ query `/services/ai-consulting/` จาก GSC (REFERENCE §6.2) → พบ "บริการให้คำปรึกษา ai solution" 124 impr pos 62 ไม่มีหน้าเจ้าของ → ใส่ใน title/H1/answer ของหน้าบริการ (page 44, RM meta ตั้งแล้ว) + H2 AI Transformation + ลิงก์ EN · query EN 5 คำ (~190 impr) ยังโชว์หน้า TH แทน /en/ → รอ /en/ index (รอบ 3) แล้วเช็ค Friday loop · (2) **หน้าเงื่อนไขการันตีฉบับเต็ม** `/services/seo/guarantee-terms/` (page 268, template page-guarantee-terms.php: นิยาม 7 คำ, ชั้น 1 เกณฑ์ 4 ข้อ + ข้อยกเว้น, ชั้น 2 วิธีนับ + ตัวอย่าง, เงื่อนไข 7 ข้อ, ขั้นตอนเมื่อไม่ถึงเป้า, FAQ 6) ลิงก์จาก /services/seo/#guarantee + /en/seo/ · ตัวเลขใหม่ที่เพิ่มเพื่อให้สัญญาใช้ได้จริง (Tum ตรวจ): baseline ต่ำกว่า 200 impr/28 วัน = KPI รายเคส · เลื่อนวันนับถ้าลูกค้าแก้เว็บช้าเกิน 10 วันทำการ · เหตุสุดวิสัย (เว็บล่ม >72 ชม., ถูกแฮ็ก) ไม่นับ · TTFB โฮสต์ >1.5s เป็นข้อยกเว้นชั้น 1 · ไม่คืนเงิน 3 เดือนแรก
- ✅ **Tum "ทำได้เลย" (2026-08-29 ดึก):** เงื่อนไขการันตีฉบับเต็มอนุมัติตามที่เขียน · `/en/website-development/` (page 269, template page-en-website-development.php — ตารางราคา 4 tier จาก TH page, included 6, process 6, proof (98 / 100% index / 95+ guarantee), FAQ 8, Service + OfferCatalog schema) · hreflang คู่ที่ 4 · EN meta · ลิงก์จาก /services/ EN card, TH website-dev hero, /en/seo/, /en/ai-search/ · schema markup post = post 81 มีอยู่แล้ว → KEYWORD-DB ชี้ไปที่ post 81 ไม่ทำโพสต์ซ้ำ · KEYWORD-DB status reconcile: todo เหลือ 1 แถว (javascript seo คือ score 19 — low) · EN ครบ 4 บริการหลัก: ai-consulting / seo / ai-search / website-development (n8n ไม่มี demand EN)
...
- **รอบ 3 (30 ส.ค. 15:37 — cron dc26a624):** /en/seo/ → /en/ai-search/ → /en/website-development/ → /services/seo/guarantee-terms/ → n8n (Google ไม่รู้จัก URL!) → /en/ai-consulting/ → post 264 → 265 → 250 → 251 → /services/ai-consulting/ (quota ~10; 245/246/249 อยู่ใน post-sitemap ที่ Google อ่านแล้ว 29 ส.ค. — ปล่อยให้ crawl เอง)
- **รอบ 3 ผลจริง (31 ส.ค. ~12:00, cron เลื่อนจาก 30 ส.ค.):** ทุก URL curl 200 ก่อนส่ง · ✅ requested 7: /en/seo/ (found-not-indexed) · /en/website-development/ (ไม่รู้จัก) · /services/seo/guarantee-terms/ (ไม่รู้จัก) · /services/n8n-automation/ (ไม่รู้จัก — ตกจาก discovered!) · post 264 ai-mode (ไม่รู้จัก) · post 265 aeo (ไม่รู้จัก) · post 250 บริษัทรับทำ-seo (ไม่รู้จัก) · ✓ indexed แล้ว ข้าม 2: /en/ai-search/, /en/ai-consulting/ · ✗ quota เกินก่อนถึง: post 251, /services/ai-consulting/ (indexed อยู่แล้วจากรอบ 1) → รอบหน้าเหลือแค่ post 251 · เทคนิค: search box หลัง navigate คลิกครั้งแรกไม่โฟกัส ต้องคลิกซ้ำ; toast ค้าง ~15s บังช่อง
- **PSI mobile 31 ส.ค. ~12:10 (pagespeed.web.dev UI):** `/en/website-development/` **99** / a11y 96 / BP 100 / SEO 100 / Agentic 3/3 (LCP 1.7s, TBT 10ms, CLS 0) · `/services/seo/guarantee-terms/` **98** / 100 / 100 / 100 / 3/3 (LCP 2.1s, TBT 20ms, CLS 0) · CrUX ยัง No Data ทั้งคู่
- **31 ส.ค. ~15:00 ย้าย listicle EN (Tum: "เพราะเป็นหน้า en"):** post 251 → **page 270** `/en/ai-consulting-companies-thailand-2026/` (parent 100, เนื้อหาจาก dist/17.json รวม FAQ block, RM meta เดิม) · post 251 = draft · 301 เพิ่มใน `hashbox_redirect_legacy_ai_guides` · `$en_meta` + ลิงก์ใน page-en-ai-consulting/page-en-ai-search แก้แล้ว (commit cba761d — cherry-pick จาก 5c9e8aa ที่หลงไป branch fix/gtm-website-lead-v1) · เหตุผล: ยังไม่ index = ย้ายฟรี, ได้ nav/footer EN อัตโนมัติ + filter /en/ ใน GSC · post 92 index แล้ว ไม่ย้าย · บั๊กที่เจอตอนย้าย: `hashbox_page_is_english()` exact-match กับ hreflang pairs → หน้า /en/ ใหม่ได้ nav ไทย + lang=th-TH — แก้เป็น prefix `en/` (1896f57) ตอนนี้ทุกหน้าใต้ /en/ ได้ lang=en อัตโนมัติ · sitemap flush key 20260831a (page 27 / post 25 URL) · GSC ขอ index หน้าใหม่รอบหน้า (quota เกินวันนี้)
- **31 ส.ค. เย็น — batch 4 โพสต์ (Tum อนุมัติ "ทำตามที่แนะนำทั้งหมด"):** post 272 **ค่าจ้าง SEO 2026** (/ค่าจ้าง-seo-2026/ — pricing guide, evidence ราคาเปิด 4 ราย จาก research post 250, ลิงก์ guarantee-terms) · post 273 **ฟอนต์ไทยฟรี** (/ฟอนต์ไทยฟรี-ทำเว็บ-2026/ — 15 ฟอนต์ + performance, target ฟ้อนต์ 201k ยาก 13 anga #8) · post 274 **สมัคร Gmail** (/สมัคร-gmail-2026/ — EXPERIMENT 368k ยาก 1, business angle Workspace; เงื่อนไข: Friday loop 6–8 สัปดาห์ไม่เข้า top-20 = หยุดสาย volume) · post 275 **Perplexity AI** (/perplexity-ai-คืออะไร-2026/ — C4 cluster + 6 ข้อ get-cited) · ทุกตัวมี brief ใน content/briefs/ + dist 20/22/23/24.json (commit c6cc6a4) · date trap ซ้ำ: สร้างแล้ว future → PATCH publish ย้อน 30 ส.ค. · **ค้าง:** listicle "10 บริษัทรับทำเว็บไซต์" (ต้อง research คู่แข่งจริง) + TH AI-consulting listicle + SEO สายเทา post + n8n ฟรีไหม FAQ + GSC request index 4 โพสต์ใหม่ (quota พรุ่งนี้: post 251 เดิมเปลี่ยนเป็น page 270 + 4 ตัวนี้)
- **31 ส.ค. ค่ำ — batch 2 (3 โพสต์):** post 281 **10 บริษัทรับทำเว็บไซต์** (/บริษัทรับทำเว็บไซต์-2026/ — research จริง 12 เว็บด้วย 3 subagents ทุก claim มี quote+URL; 7 เกณฑ์; Hashbox 6/7, CW 4/7, DAB 4/7; ตกรอบ Morphosis/1stCraft/SoGoodWeb; เจอ ZMYWEB ระบุลิขสิทธิ์ design เป็นของบริษัทตัวเอง = red flag ตัวอย่างจริง) · post 283 **SEO สายเทา vs สายขาว** (/seo-สายเทา-vs-สายขาว-2026/ — spam policies 2024-2026, 5 คำถามเช็คเอเจนซี, ลิงก์ขึ้น white-hat) · post 285 **บริษัทที่ปรึกษา AI ไทย** (/บริษัทที่ปรึกษา-ai-ไทย-2026/ — TH ของ page 270 ใช้ research ชุด 29 ส.ค., กล่าวถึง VISAI ตอบ demand คำถาม ×11) · dist 21/25/26 commit 2a13400 · sitemap flush key 20260831c (post-sitemap 32) · **งานคิว Tum อนุมัติ "ทำทั้งหมด" ครบแล้ว** · GSC พรุ่งนี้: page 270 + posts 272/273/274/275/281/283/285 (8 URL — quota 10/วัน พอ)
- **รอบ 4 (1 ก.ย. ~04:00):** post 285 เพิ่ม H2+FAQ "บริษัทใดบ้างที่ทำ AI ในประเทศไทย?" (query ที่ Tum ต้องการติด — ตาราง 4 ประเภท ecosystem, commit ec16fb6) · GSC: /en/ai-consulting-companies-thailand-2026/ **index แล้ว** (เร็ว — ผล 301+sitemap) · /ค่าจ้าง-seo-2026/ requested ✓ · **quota เกินหลัง 1 คำขอ** (rolling 24h — เมื่อวานใช้ 7 ตอนเที่ยง) · ค้าง 6: ฟอนต์, gmail, perplexity, บริษัทรับทำเว็บไซต์, สายเทา, บริษัทที่ปรึกษา ai → รอบต่อไปหลังเที่ยง 1 ก.ย.
- **1 ก.ย. บ่าย — batch 3 (ตอบคำถามจาก Signal ที่ Tum ส่ง):** post 290 **มือใหม่ทำ SEO** · 292 **SEO vs SEM** · 294 **AI ฟรีช่วยทำงาน** · 296 **ตัวอย่างบทความ SEO** (ใช้บทความเราเองเป็นตัวอย่าง — meta play) + FAQ EN 3 จุด: page-en-seo "Which SEO agency best" · page 270 "What AI available" + "high demand" (dist/17 v2) · ข้าม "จบ AI เงินเดือน" (คนหางาน ไม่ใช่ audience) · commit 3e34aa8 — branch หลักกลับมาเป็น main แล้ว push ตรงได้ · คำถามครบ 15/16 จากตาราง Signal 2026-09-01
